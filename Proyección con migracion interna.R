...
dev.off()
#par(mfrow=c(1,2))
matplot(log(mx[111:220,]),type="l", 
        xlab = "Edad",
        main="Hombres",
        ylab="log(mx)")
matplot(log(mx[1:110,]),type="l",
        main="Mujeres",
        xlab = "Edad",
        ylab="log(mx)")
...
# Tasas constantes de migracion nacional

length(ixNH[,1])
ixNH2<-matrix(0,111,25)
ixNH2[1:91,]<-ixNH[,1]
ixNH<-ixNH2

ixNM2<-matrix(0,111,25)
ixNM2[1:91,]<-ixNM[,1]
ixNM<-ixNM2

exNH2<-matrix(0,111,25)
exNH2[1:91,]<-exNH[,1]
exNH<-exNH2

exNM2<-matrix(0,111,25)
exNM2[1:91,]<-exNM[,1]
exNM<-exNM2
...
for (i in 2:26) {
  ...
  # Edades intermedias  
  PxF.for[2:109,i] <- (PxF.for[1:108,i-1] + NxF.for[1:108,i-1]*0.5*ixF.for[1:108,i-1]+NxF.for[1:108,i-1]*0.5*ixNM[1:108,1]) * SxF.for[1:108,i-1] +
    NxF.for[2:109,i-1]*0.5*ixF.for[2:109,i-1] +
    NxF.for[2:109,i-1]*0.5*ixNM[2:109,1]-
    NxF.for[1:108,i-1]*exF.for[1:108,i-1]-
    NxF.for[1:108,i-1]*exNM[1:108,1]
  ...
  PxF.for[110,i] <- (PxF.for[109,i-1]+
                       0.5*NxF.for[109,i-1]*ixF.for[109,i-1]+
                       0.5*NxF.for[109,i-1]*ixNM[109,1])* SxF.for[109,i-1] +
    NxF.for[110,i-1]*0.5*ixF.for[110,i-1]+
    NxF.for[110,i-1]*0.5*ixNM[110,1]-
    NxF.for[109,i-1]*0.5*ixF.for[109,i-1]-
    NxF.for[109,i-1]*0.5*ixNM[109,1]+
    (PxF.for[110,i-1] + 
       0.5*NxF.for[110,i-1]*ixF.for[110,i-1]+ 
       0.5*NxF.for[110,i-1]*ixNM[110,1])* SxF.for[110,i-1]-
    NxF.for[110,i-1]*exF.for[110,i-1]-
    NxF.for[110,i-1]*exNM[110,1]
  ...
  Bx[,i-1] <- fx.for[,i-1]*(PxF.for[16:50,i-1]+
                              ...
                              0.5*NxF.for[16:50,i-1]*ixNM[16:50,1]+
                              PxF.for[16:50,i])*0.5
  ...
  PxF.for[1,i] <- BF[1]*SxF.for[1,i-1]+ 
                  ...
                  NxF.for[1,i-1]*0.5*ixNM[1,1]+ 
                  NxF.for[1,i-1]*exF.for[1,i-1]+ 
                  NxF.for[1,i-1]*exNM[1,1]
...
for (i in 2:26) {
  ...
  PxM.for[2:109,i] <- (PxM.for[1:108,i-1] + 
                         NxM.for[1:108,i-1]*0.5*ixM.for[1:108,i-1]+ 
                         NxM.for[1:108,i-1]*0.5*ixNH[1:108,1]) * SxM.for[1:108,i-1] +
    NxM.for[2:109,i-1]*0.5*ixM.for[2:109,i-1]+
    NxM.for[2:109,i-1]*0.5*ixNH[2:109,1] -
    NxM.for[1:108,i-1]*exM.for[1:108,i-1]-
    NxM.for[1:108,i-1]*exNH[1:108,1]
  ...
  PxM.for[110,i] <- (PxM.for[109,i-1]+
                       0.5*NxM.for[109,i-1]*ixM.for[109,i-1]+
                       0.5*NxM.for[109,i-1]*ixNH[109,1])* SxM.for[109,i-1] +
    NxM.for[110,i-1]*0.5*ixM.for[110,i-1]+
    NxM.for[110,i-1]*0.5*ixNH[110,1]-
    NxM.for[109,i-1]*0.5*ixM.for[109,i-1]-
    NxM.for[109,i-1]*0.5*ixNH[109,1]+
    (PxM.for[110,i-1] + 
       0.5*NxM.for[110,i-1]*ixM.for[110,i-1]+ 
       0.5*NxM.for[110,i-1]*ixNH[110,1])* SxM.for[110,i-1]-
    NxM.for[110,i-1]*exM.for[110,i-1]-
    NxM.for[110,i-1]*exNH[110,1]
  ...
  PxM.for[1,i] <- BM[1]*SxM.for[1,i-1]+ 
    ...
    NxM.for[1,i-1]*0.5*ixNH[1,1]+ 
    NxM.for[1,i-1]*exM.for[1,i-1]+ 
    NxM.for[1,i-1]*exNH[1,1]
...
dev.off()
par(mfrow=c(1,2))
matplot(log(mx.for[1:110,]),type="l", main="Hombres",xlab="Edades",ylab="log(mx)")
matplot(log(mx.for[111:220,]),type="l",main="Mujeres",xlab="Edades",ylab="log(mx)")

dev.off()
par(mfrow=c(1,2))
matplot(c(15:49),(fx.for),type="l",xlab="Edades",ylab="Tasa específica")
matplot(c(2016:2040),colSums(fx.for),type="l",xlab="Año",ylab="Tasa global de fecundidad")

dev.off()
par(mfrow=c(1,2))
matplot((ixM.for),type="l",xlab="Edades",ylab="Tasa", main="Hombres")
matplot((ixF.for),type="l",xlab="Edades",ylab="Tasa", main="Mujeres")

dev.off()
par(mfrow=c(1,2))
matplot((exM.for),type="l",xlab="Edades",ylab="Tasa", main="Hombres")
matplot((exF.for),type="l",xlab="Edades",ylab="Tasa", main="Mujeres")

dev.off()
par(mfrow=c(1,2))
matplot(c(2015:2040),colSums(NxM.for)/10000,type="l",xlab="Año",ylab="(x10,000)", main="Hombres")
matplot(c(2015:2040),colSums(NxF.for)/10000,type="l",xlab="Año",ylab="(x10,000)", main="Mujeres")


# Piramides de poblacion 

nproyF.prop<-matrix(0,110,26)
nproyM.prop<-matrix(0,110,26)

for (i in 1:25) {
  nproyF.prop[,i]<-NxF.for[,i]/colSums(NxF.for+NxM.for)[i]
  nproyM.prop[,i]<-NxM.for[,i]/colSums(NxF.for+NxM.for)[i]
}

## Paleta de colores para daltónicos
pal <- brewer.pal(n = 9, name = 'BrBG')

## Llenando las pirámides

for (i in 1:25) {
  yr<-i  
  jpeg(file=paste("piram_",2015+i,".jpeg",sep = ""))
  #mover los margenes
  par(mar=(c(1,1,1,1)))
  pyramid(as.data.frame(cbind(nproyM.prop[,yr],nproyM.prop[,yr],c(0:109))),Cstep =5,Llab = "Hombres",Rlab = "Mujeres",Clab = "Edad",Rcol = pal[3], Lcol = pal[8], main =paste("Veracruz", i+2015,sep = " "))
  dev.off()
}
